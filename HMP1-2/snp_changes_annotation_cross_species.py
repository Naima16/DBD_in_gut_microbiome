...
for species_name in good_species_list:
		print species_name
		if os.path.isfile("%s/gene_changes_test/gene_changes_anno_old_%s.pkl" % (pdir, species_name)):
				all_data_species = pickle.load( open( "%s/gene_changes_test/gene_changes_anno_old_%s.pkl" % (pdir, species_name), "rb" ))
				if (len(all_data_species['hmp'][species_name].keys()) >0): # check if there were any gene changes to be outputted. 
						all_data[species_name]=all_data_species['hmp'][species_name]
...
outFile_gene_change=open('%ssnp_changes_accross_species.txt' %  config.analysis_directory,'w')
...
outFile_keywords=open('%ssnp_changes_accross_species_keywords.txt' %  config.analysis_directory,'w')
