...
# Construct final tables from pickles from pickle_diversity_vs_temporal_changes.py
...
sample_day_map = su.parse_sample_day_map()
...
# ====================================================================
# Polymorphism per sample-species pair table (Poyet)
# Also includes alpha diversity and richness
# ====================================================================

analysis_dir = config.analysis_directory
analysis_dir = '%s/sanity_check' % analysis_dir
with open("%s/Poyet_polymorphism.csv" % analysis_dir, 'w') as outfile:
	
	fields = ','.join('sample_id subject tp species alpha_div alpha_div_rare richness richness_rare polymorphism'.split())
	...
	for sample in sample_species_polymorphism_dict:
		for species in sample_species_polymorphism_dict[sample]:
			...
			tp = sample_day_map[sample] # Use day as timepoint
			...
			fields_data = [sample, subject, tp, species, alpha_div, alpha_div_rarefied, richness, richness_rarefied, polymorphism]
			
			outfile.write(','.join([str(f) for f in fields_data]) + '\n')

# ====================================================================
# Delta polymorphism for consecutive timepoint pairs (Poyet)
# ====================================================================

desired_samples = sample_species_polymorphism_dict.keys()
same_subject_idxs = su.calculate_ordered_same_subject_pairs(sample_order_map, desired_samples, within_host_type='nonconsecutive')

analysis_dir = config.analysis_directory
analysis_dir = '%s/sanity_check' % analysis_dir
with open("%s/Poyet_delta_polymorphism.csv" % analysis_dir, 'w') as outfile:
...
with open("%s/Poyet_gene_changes_full.csv" % analysis_dir, 'w') as outfile:
	...
	for sample_i, sample_j in gene_gain_dict:
		for species in gene_gain_dict[(sample_i, sample_j)]:
			...
			subject = sample_subject_map[sample_i]
			subject1, order1 = sample_order_map[sample_i]
			tp1 = sample_day_map[sample_i] # Use days as timepoint
			subject2, order2 = sample_order_map[sample_j]
			tp2 = sample_day_map[sample_j] # Use days as timepoint
			if subject1 != subject2 or subject != subject1:
				print("Weird")
