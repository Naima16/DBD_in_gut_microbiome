...
gene_gain_info_dict = defaultdict(dict)
gene_loss_info_dict = defaultdict(dict)
...
for species_name in good_species_list:
        ...
        for desired_samples, same_subject_idxs, status in zip([qp_samples, all_samples, all_samples], [qp_same_subject_idxs, nonqp_same_subject_idxs, all_same_subject_idxs], ['qp', 'nonqp', 'all']):
                ...
                for sample_pair_idx in xrange(0,len(same_subject_idxs[0])):
                        ...
                        gene_gain_info_dict[(sample_i, sample_j)][species_name] = gains
                        gene_loss_info_dict[(sample_i, sample_j)][species_name] = losses
...
pickle.dump(gene_gain_info_dict, open("%s/gene_gain_info_dict.pkl" % (pickle_dir), 'wb'))
pickle.dump(gene_loss_info_dict, open("%s/gene_loss_info_dict.pkl" % (pickle_dir), 'wb'))
'''
