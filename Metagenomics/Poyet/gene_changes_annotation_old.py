...
for species_name in good_species_list: 
	...
	for cohort in cohorts:
		...
		# Analyze the data #
		####################
		# Just dealing with one cohort, HMP
		haploid_samples_cohort = list(set(haploid_samples).intersection(set(su.get_sample_names())))
		...
		same_sample_idxs, same_subject_idxs, diff_subject_idxs = su.calculate_ordered_subject_pairs(sample_order_map, haploid_samples)
		
		# Sanity check
		# for i, j in zip(same_subject_idxs[0], same_subject_idxs[1]):
		#   print(sample_order_map[haploid_samples[i]])
		#   print(sample_order_map[haploid_samples[j]])
...
pickle.dump( all_data, open( "%s/gene_changes_test/gene_changes_anno_old_%s.pkl" % (pdir, species_name), "wb" ) )
